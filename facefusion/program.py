...
def get_config_str_list_safe(section : str, option : str, choices : List[str], fallback : str = None) -> List[str]:
	values = config.get_str_list(section, option, fallback)
	valid_values = [ value for value in values if value in choices ]
	if not valid_values and choices:
		return [ get_first(choices) ]
	return valid_values


def get_config_str_value_safe(section : str, option : str, choices : List[str], fallback : str = None) -> str:
	value = config.get_str_value(section, option, fallback)
...
def create_face_detector_program() -> ArgumentParser:
	...
	group_face_detector.add_argument('--face-detector-model', help = translator.get('help.face_detector_model'), default = get_config_str_value_safe('face_detector', 'face_detector_model', facefusion.choices.face_detector_models, 'yolo_face'), choices = facefusion.choices.face_detector_models)
	known_args, _ = program.parse_known_args()
	face_detector_size_choices = facefusion.choices.face_detector_set.get(known_args.face_detector_model)
	group_face_detector.add_argument('--face-detector-size', help = translator.get('help.face_detector_size'), default = get_config_str_value_safe('face_detector', 'face_detector_size', face_detector_size_choices, get_last(face_detector_size_choices)), choices = face_detector_size_choices)
...
def create_face_landmarker_program() -> ArgumentParser:
	...
	group_face_landmarker.add_argument('--face-landmarker-model', help = translator.get('help.face_landmarker_model'), default = get_config_str_value_safe('face_landmarker', 'face_landmarker_model', facefusion.choices.face_landmarker_models, '2dfan4'), choices = facefusion.choices.face_landmarker_models)
...
def create_face_selector_program() -> ArgumentParser:
	...
	group_face_selector.add_argument('--face-selector-mode', help = translator.get('help.face_selector_mode'), default = get_config_str_value_safe('face_selector', 'face_selector_mode', facefusion.choices.face_selector_modes, 'reference'), choices = facefusion.choices.face_selector_modes)
	group_face_selector.add_argument('--face-selector-order', help = translator.get('help.face_selector_order'), default = get_config_str_value_safe('face_selector', 'face_selector_order', facefusion.choices.face_selector_orders, 'large-small'), choices = facefusion.choices.face_selector_orders)
	...
	group_face_selector.add_argument('--face-selector-gender', help = translator.get('help.face_selector_gender'), default = get_config_str_value_safe('face_selector', 'face_selector_gender', facefusion.choices.face_selector_genders), choices = facefusion.choices.face_selector_genders)
	group_face_selector.add_argument('--face-selector-race', help = translator.get('help.face_selector_race'), default = get_config_str_value_safe('face_selector', 'face_selector_race', facefusion.choices.face_selector_races), choices = facefusion.choices.face_selector_races)
	...
	group_face_selector.add_argument('--face-selector-region', help = translator.get('help.face_selector_region'), type = int, default = config.get_int_list('face_selector', 'face_selector_region', '0 0 0 0'), nargs = 4)
	job_store.register_step_keys([ 'face_selector_mode', 'face_selector_order', 'face_selector_gender', 'face_selector_race', 'face_selector_age_start', 'face_selector_age_end', 'reference_face_position', 'reference_face_distance', 'reference_frame_number', 'face_selector_region' ])
	return program
...
def create_face_masker_program() -> ArgumentParser:
	...
	group_face_masker.add_argument('--face-occluder-model', help = translator.get('help.face_occluder_model'), default = get_config_str_value_safe('face_masker', 'face_occluder_model', facefusion.choices.face_occluder_models, 'xseg_1'), choices = facefusion.choices.face_occluder_models)
	group_face_masker.add_argument('--face-parser-model', help = translator.get('help.face_parser_model'), default = get_config_str_value_safe('face_masker', 'face_parser_model', facefusion.choices.face_parser_models, 'bisenet_resnet_34'), choices = facefusion.choices.face_parser_models)
	group_face_masker.add_argument('--face-mask-types', help = translator.get('help.face_mask_types').format(choices = ', '.join(facefusion.choices.face_mask_types)), default = get_config_str_list_safe('face_masker', 'face_mask_types', facefusion.choices.face_mask_types, 'box'), choices = facefusion.choices.face_mask_types, nargs = '+', metavar = 'FACE_MASK_TYPES')
	group_face_masker.add_argument('--face-mask-areas', help = translator.get('help.face_mask_areas').format(choices = ', '.join(facefusion.choices.face_mask_areas)), default = get_config_str_list_safe('face_masker', 'face_mask_areas', facefusion.choices.face_mask_areas, ' '.join(facefusion.choices.face_mask_areas)), choices = facefusion.choices.face_mask_areas, nargs = '+', metavar = 'FACE_MASK_AREAS')
	group_face_masker.add_argument('--face-mask-regions', help = translator.get('help.face_mask_regions').format(choices = ', '.join(facefusion.choices.face_mask_regions)), default = get_config_str_list_safe('face_masker', 'face_mask_regions', facefusion.choices.face_mask_regions, ' '.join(facefusion.choices.face_mask_regions)), choices = facefusion.choices.face_mask_regions, nargs = '+', metavar = 'FACE_MASK_REGIONS')
	...
	group_face_masker.add_argument('--face-mask-erosion', help = translator.get('help.face_mask_erosion'), type = float, default = config.get_float_value('face_masker', 'face_mask_erosion', '0.0'), choices = facefusion.choices.face_mask_erosion_range, metavar = create_float_metavar(facefusion.choices.face_mask_erosion_range))
	job_store.register_step_keys([ 'face_occluder_model', 'face_parser_model', 'face_mask_types', 'face_mask_areas', 'face_mask_regions', 'face_mask_blur', 'face_mask_padding', 'face_mask_erosion' ])
	return program
...
def create_voice_extractor_program() -> ArgumentParser:
	...
	group_voice_extractor.add_argument('--voice-extractor-model', help = translator.get('help.voice_extractor_model'), default = get_config_str_value_safe('voice_extractor', 'voice_extractor_model', facefusion.choices.voice_extractor_models, 'kim_vocal_2'), choices = facefusion.choices.voice_extractor_models)
...
def create_frame_extraction_program() -> ArgumentParser:
	...
	group_frame_extraction.add_argument('--temp-frame-format', help = translator.get('help.temp_frame_format'), default = get_config_str_value_safe('frame_extraction', 'temp_frame_format', facefusion.choices.temp_frame_formats, 'png'), choices = facefusion.choices.temp_frame_formats)
...
def create_output_creation_program() -> ArgumentParser:
	...
	group_output_creation.add_argument('--output-audio-encoder', help = translator.get('help.output_audio_encoder'), default = get_config_str_value_safe('output_creation', 'output_audio_encoder', available_encoder_set.get('audio'), get_first(available_encoder_set.get('audio'))), choices = available_encoder_set.get('audio'))
	...
	group_output_creation.add_argument('--output-video-encoder', help = translator.get('help.output_video_encoder'), default = get_config_str_value_safe('output_creation', 'output_video_encoder', available_encoder_set.get('video'), get_first(available_encoder_set.get('video'))), choices = available_encoder_set.get('video'))
	group_output_creation.add_argument('--output-video-preset', help = translator.get('help.output_video_preset'), default = get_config_str_value_safe('output_creation', 'output_video_preset', facefusion.choices.output_video_presets, 'veryfast'), choices = facefusion.choices.output_video_presets)
...
def create_uis_program() -> ArgumentParser:
	...
	group_uis.add_argument('--ui-workflow', help = translator.get('help.ui_workflow'), default = get_config_str_value_safe('uis', 'ui_workflow', facefusion.choices.ui_workflows, 'instant_runner'), choices = facefusion.choices.ui_workflows)
...
def create_download_providers_program() -> ArgumentParser:
	...
	group_download.add_argument('--download-providers', help = translator.get('help.download_providers').format(choices = ', '.join(facefusion.choices.download_providers)), default = get_config_str_list_safe('download', 'download_providers', facefusion.choices.download_providers, ' '.join(facefusion.choices.download_providers)), choices = facefusion.choices.download_providers, nargs = '+', metavar = 'DOWNLOAD_PROVIDERS')
...
def create_download_scope_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	group_download = program.add_argument_group('download')
	group_download.add_argument('--download-scope', help = translator.get('help.download_scope'), default = get_config_str_value_safe('download', 'download_scope', facefusion.choices.download_scopes, 'lite'), choices = facefusion.choices.download_scopes)
...
def create_benchmark_program() -> ArgumentParser:
	...
	group_benchmark.add_argument('--benchmark-mode', help = translator.get('help.benchmark_mode'), default = get_config_str_value_safe('benchmark', 'benchmark_mode', facefusion.choices.benchmark_modes, 'warm'), choices = facefusion.choices.benchmark_modes)
	group_benchmark.add_argument('--benchmark-resolutions', help = translator.get('help.benchmark_resolutions'), default = get_config_str_list_safe('benchmark', 'benchmark_resolutions', facefusion.choices.benchmark_resolutions, get_first(facefusion.choices.benchmark_resolutions)), choices = facefusion.choices.benchmark_resolutions, nargs = '+')
...
def create_execution_program() -> ArgumentParser:
	...
	exe_providers_default = get_config_str_list_safe('execution', 'execution_providers', available_execution_providers, get_first(available_execution_providers))
	group_execution.add_argument('--execution-providers', help = translator.get('help.execution_providers').format(choices = ', '.join(available_execution_providers)), default = exe_providers_default, nargs = '+', metavar = 'EXECUTION_PROVIDERS')
...
def create_memory_program() -> ArgumentParser:
	...
	group_memory.add_argument('--video-memory-strategy', help = translator.get('help.video_memory_strategy'), default = get_config_str_value_safe('memory', 'video_memory_strategy', facefusion.choices.video_memory_strategies, 'strict'), choices = facefusion.choices.video_memory_strategies)
...
def create_log_level_program() -> ArgumentParser:
	...
	group_misc.add_argument('--log-level', help = translator.get('help.log_level'), default = get_config_str_value_safe('misc', 'log_level', facefusion.choices.log_levels, 'info'), choices = facefusion.choices.log_levels)
	job_store.register_job_keys([ 'log_level' ])
	return program


def create_diagnostics_program() -> ArgumentParser:
	program = ArgumentParser(add_help = False)
	group_diagnostics = program.add_argument_group('diagnostics')
	group_diagnostics.add_argument('--diagnostics-path', help = translator.get('help.diagnostics_path'), default = 'facefusion_diagnostics.json')
	job_store.register_job_keys([ 'diagnostics_path' ])
	return program
...
def create_program() -> ArgumentParser:
	...
	program.add_argument('--config-path', help = translator.get('help.config_path'), default = 'facefusion.ini')
	...
	sub_program.add_parser('diagnostics-run', help = translator.get('help.diagnostics_run'), parents = [ create_diagnostics_program(), create_log_level_program() ], formatter_class = create_help_formatter_large)
	# job manager
	...
	# misc
	sub_program.add_parser('force-download', help = translator.get('help.force_download'), parents = [ create_log_level_program(), create_download_scope_program() ], formatter_class = create_help_formatter_large)
...
def apply_config_path(program : ArgumentParser) -> None:
	...
	if '--config-path' in sys.argv:
		index = sys.argv.index('--config-path') + 1
		if index < len(sys.argv):
			config_path = sys.argv[index]
