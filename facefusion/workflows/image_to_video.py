...
def extract_frames() -> ErrorCode:
	...
	trim_frame_start, trim_frame_end = restrict_trim_frame(state_manager.get_item('target_path'), state_manager.get_item('trim_frame_start'), state_manager.get_item('trim_frame_end'))
	target_resolution = detect_video_resolution(state_manager.get_item('target_path')) or (0, 0)
	output_video_resolution = scale_resolution(target_resolution, state_manager.get_item('output_video_scale'))
	...
	logger.info(get_translation('extracting_frames').format(resolution=pack_resolution(temp_video_resolution), fps=temp_video_fps), __name__)
	...
	if ffmpeg.extract_frames(state_manager.get_item('target_path'), temp_video_resolution, temp_video_fps, trim_frame_start, trim_frame_end):
		logger.debug(get_translation('extracting_frames_succeeded'), __name__)
	else:
		if is_process_stopping():
			return 4
		logger.error(get_translation('extracting_frames_failed'), __name__)
		return 1
	return 0
...
def process_video() -> ErrorCode:
	...
	if temp_frame_paths:
		...
		with tqdm(total = len(temp_frame_paths), desc = get_translation('processing'), unit = 'frame', ascii = ' =', disable = state_manager.get_item('log_level') in [ 'warn', 'error' ]) as progress:
		...
		logger.error(get_translation('temp_frames_not_found'), __name__)
		return 1
	return 0
...
def merge_frames() -> ErrorCode:
	...
	logger.info(get_translation('merging_video').format(resolution = pack_resolution(output_video_resolution), fps = state_manager.get_item('output_video_fps')), __name__)
	if ffmpeg.merge_video(state_manager.get_item('target_path'), temp_video_fps, output_video_resolution, state_manager.get_item('output_video_fps'), trim_frame_start, trim_frame_end):
		logger.debug(get_translation('merging_video_succeeded'), __name__)
	else:
		if is_process_stopping():
			return 4
		logger.error(get_translation('merging_video_failed'), __name__)
		return 1
	return 0
...
def restore_audio() -> ErrorCode:
	...
	if state_manager.get_item('output_audio_volume') == 0:
		logger.info(get_translation('skipping_audio'), __name__)
		...
		if source_audio_path:
			if ffmpeg.replace_audio(state_manager.get_item('target_path'), source_audio_path, state_manager.get_item('output_path')):
				...
				logger.debug(get_translation('replacing_audio_succeeded'), __name__)
			else:
				video_manager.clear_video_pool()
				if is_process_stopping():
					return 4
				logger.warn(get_translation('replacing_audio_skipped'), __name__)
				move_temp_file(state_manager.get_item('target_path'), state_manager.get_item('output_path'))
		else:
			if ffmpeg.restore_audio(state_manager.get_item('target_path'), state_manager.get_item('output_path'), trim_frame_start, trim_frame_end):
				video_manager.clear_video_pool()
				logger.debug(get_translation('restoring_audio_succeeded'), __name__)
			else:
				video_manager.clear_video_pool()
				if is_process_stopping():
					return 4
				logger.warn(get_translation('restoring_audio_skipped'), __name__)
				move_temp_file(state_manager.get_item('target_path'), state_manager.get_item('output_path'))
	return 0
...
def finalize_video(start_time : float) -> ErrorCode:
	...
	if is_video(state_manager.get_item('output_path')):
		logger.info(get_translation('processing_video_succeeded').format(seconds = calculate_end_time(start_time)), __name__)
	else:
		logger.error(get_translation('processing_video_failed'), __name__)
		return 1
	return 0
